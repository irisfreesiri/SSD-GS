...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, unfreeze_iterations, debug_from):
    ...
    if checkpoint:
        (model_params, camera_params, first_iter) = torch.load(checkpoint)
        gaussians.restore(model_params, opt)
        scene.restore(camera_params)

    bg_color = [1, 1, 1] + [0] * (13 - 3) if dataset.white_background else [0] * 13
    ...
    while iteration <= opt.iterations:
        ...
        gaussians.update_learning_rate(iteration, \
                                        ...
                                        freeze_scatter_steps=opt.scatter_lr_freeze_step)
        ...
        asg_sigmas = gaussians.asg_func.get_asg_sigma  # (basis_asg_num, 1)
        ...
        disable_scatter = iteration < opt.scatter_warmup_step
        disable_specular = iteration < opt.specular_freeze_step + opt.fit_linear_step
        disable_shadow = iteration <= unfreeze_iterations
        is_train = prune_visibility and not opt_test
        render_pkg = render(viewpoint_cam, gaussians, light_stream, calc_stream, local_axises, asg_scales, asg_sigmas, asg_axises, pipe, bg,
                            disable_specular=disable_specular, disable_scatter=disable_scatter, disable_shadow=disable_shadow, is_train=is_train)
        ...
        if dataset.hdr:
            if iteration <= opt.specular_freeze_step:
                gamma = 2.2
                gt_image = torch.pow(gt_image, 1./gamma)
            elif iteration < opt.specular_freeze_step + opt.fit_linear_step//2:
                gamma = 1.1 * float(opt.specular_freeze_step + opt.fit_linear_step - iteration + 1) / float(opt.fit_linear_step // 2 + 1)
                gt_image = torch.pow(gt_image, 1./gamma)
        else:
            render_image = torch.clip(render_image, 0.0, 1.0)
            
        # Loss
        Ll1 = l1_loss(render_image, gt_image)
        SSIM = ssim(render_image, gt_image)
        ...
        with torch.no_grad():
            if not opt_test:
                ...
                if iteration < opt.densify_until_iter:
                    ...
                    gaussians.add_densification_stats(viewspace_point_tensor, visibility_filter, render_image.shape[2], render_image.shape[1], render_pkg["out_weight"])
            ...
            if not opt_test:
                if (iteration in checkpoint_iterations):
                    ...
                    torch.save((gaussians.capture(), scene.capture(), iteration), scene.model_path + "/chkpnt" + str(iteration) + ".pth")

            if iteration == opt.specular_freeze_step:
                gaussians.neural_phasefunc.freeze()
                gaussians.neural_material.requires_grad_(False)
            
            if iteration == opt.specular_freeze_step + opt.fit_linear_step:
                gaussians.neural_phasefunc.unfreeze()
                gaussians.neural_material.requires_grad_(True)

            if iteration == opt.scatter_freeze_step:
                gaussians.scatter_mlp.freeze()
            if iteration == opt.scatter_unfreeze_step:
                gaussians.scatter_mlp.unfreeze()
...
def training_report(tb_writer, iteration, Ll1, loss, l1_loss, elapsed, testing_iterations, scene : Scene, renderFunc, renderArgs, gamma=1.0):
    ...
    if iteration in testing_iterations:
        ...
        asg_sigmas = scene.gaussians.asg_func.get_asg_sigma  # (basis_asg_num, 1)
        ...
        for config in validation_configs:
            if config['cameras'] and len(config['cameras']) > 0:
                ...
                for idx, viewpoint in enumerate(config['cameras']):
                    render_pkg = renderFunc(viewpoint, scene.gaussians, light_stream, calc_stream, local_axises, asg_scales, asg_sigmas, asg_axises, *renderArgs)
                    render_image = render_pkg["render"]
                    render_image = torch.clamp(render_image, 0.0, 1.0)

                    gt_image = torch.clamp(viewpoint.original_image.to("cuda"), 0.0, 1.0)
                    if tb_writer and (idx < 5):
                        tb_writer.add_images(config['name'] + "_view_{}/render".format(viewpoint.image_name), render_image[None].pow(1./gamma), global_step=iteration)
                    ...
                    psnr_test += psnr(render_image, gt_image).mean().double()
