...
from scene.scatter_mlp import ScatterMLP
...
class GaussianModel:
    ...
    def __init__(self, 
        ...
        # Extend Physic-based SSS
        self.ksss = torch.empty(0)
        ...
        # Extend Scatter MLP
        self.scatter_mlp = torch.empty(0)
    ...
    def restore(self, model_args, training_args):
        ...
        # Extend Scatter MLP
        self.scatter_mlp.load_state_dict(scatter_mlp_param)
        print("Number of points at initialisation (restore_from_checkpoint) : ", self._xyz.shape[0])
    ...
    @property
    def get_ksss(self):
        return self.kd_activation(self.ksss)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        print("Number of points at initialisation (create_from_pcd) : ", fused_point_cloud.shape[0])
        ...
        if self.use_MBRDF:
            ...
            # Extend Physic-based SSS
            ksss = torch.ones((features.shape[0], 3), dtype=torch.float, device="cuda")*0.5
            self.ksss = nn.Parameter(ksss.requires_grad_(True))
            ...
            # Extend Scatter MLP
            self.scatter_mlp = ScatterMLP(self.neural_material_size).to(device="cuda")
    ...
    def training_setup(self, training_args):
        ...
        if self.use_MBRDF:
            ...
            # Extend Physic-based SSS
            l.append({'params': [self.ksss], 'lr': training_args.ksss_lr, "name": "ksss"})
            ...
            # Extend Physic-based SSS
            l.append({'params': self.scatter_mlp.parameters(), 'lr': training_args.scatter_lr_init, "name": "scatter"})
        ...
        # Extend Physic-based SSS
        self.scatter_mlp_schedular_args = get_expon_lr_func(lr_init=training_args.scatter_lr_init,
                                                    lr_final=training_args.scatter_lr_final,
                                                    lr_delay_mult=training_args.scatter_lr_delay_mult,
                                                    max_steps=training_args.scatter_lr_max_steps)


    def update_learning_rate(self, iteration, asg_freeze_step=0, local_q_freeze_step=0, freeze_phasefunc_steps=0, freeze_scatter_steps=0):
        ...
        for param_group in self.optimizer.param_groups:
            ...
            if param_group["name"] == "local_q":
                ...
                param_group['lr'] = lr
            if param_group["name"] in ["neural_phasefunc", "neural_material"]:
                lr = self.neural_phasefunc_scheduler_args(max(0, iteration - freeze_phasefunc_steps))
                param_group['lr'] = lr
            # Extend Physic-based SSS
            if param_group["name"] == "scatter":
                lr = self.scatter_mlp_schedular_args(max(0, iteration - freeze_scatter_steps))
                param_group['lr'] = lr
    ...
    def construct_list_of_attributes(self):
        ...
        if self.use_MBRDF:
            ...
            # Extend Physic-based SSS
            for i in range(self.ksss.shape[1]):
                l.append('ksss_{}'.format(i))
    ...
    def save_ply(self, path):
        ...
        if self.use_MBRDF:
            ...
            # Extend Physic-based SSS
            ksss = self.ksss.detach().cpu().numpy()
        ...
        if self.use_MBRDF:
            attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation, \
                                        kd, ks, ksss, alpha_asg, local_q, neural_material), axis=1)
        else:
            attributes = np.concatenate((xyz, normals, f_dc, f_rest, opacities, scale, rotation), axis=1)
        elements[:] = list(map(tuple, attributes))
        el = PlyElement.describe(elements, 'vertex')
        PlyData([el]).write(path)
        print(f"Number of points at checkpoint (save_ply): {xyz.shape[0]}, path: {path}")
    ...
    def load_ply(self, path):
        ...
        print(f"Number of points at initialisation (load_ply): {xyz.shape[0]}, path: {path}")
        ...
        if self.use_MBRDF:
            ...
            # Extend Physic-based SSS
            ksss_names = [p.name for p in plydata.elements[0].properties if p.name.startswith("ksss_")]
            ksss_names = sorted(ksss_names, key = lambda x: int(x.split('_')[-1]))
            ksss = np.zeros((xyz.shape[0], len(ksss_names)))
            for idx, attr_name in enumerate(ksss_names):
                ksss[:, idx] = np.asarray(plydata.elements[0][attr_name])
        ...
        if self.use_MBRDF:
            ...
            # Extend Physic-based SSS
            self.ksss = nn.Parameter(torch.tensor(ksss, dtype=torch.float, device="cuda").requires_grad_(True))
    ...
    def prune_points(self, mask):
        ...
        if self.use_MBRDF:
            ...
            # Extend Physic-based SSS
            self.ksss = optimizable_tensors["ksss"]
    ...
    def densification_postfix(self, new_xyz, new_features_dc, new_features_rest, new_opacities, new_scaling, new_rotation, new_kd, new_ks, new_ksss, new_alpha_asg, local_q, new_neural_material):
        d = {"xyz": new_xyz,
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        # Extend Physic-based SSS
        new_ksss = None if not self.use_MBRDF else self.ksss[selected_pts_mask].repeat(N,1)
        ...
        self.densification_postfix(new_xyz, new_features_dc, new_features_rest, new_opacity, new_scaling, new_rotation, \
            new_kd, new_ks, new_ksss, new_alpha_asg, new_local_q, new_neural_material)
    ...
    def densify_and_clone(self, grads, grad_threshold, scene_extent):
        ...
        # Extend Physic-based SSS
        new_ksss = None if not self.use_MBRDF else self.ksss[selected_pts_mask]
    ...
    def densify_and_prune(self, max_grad, min_opacity, extent, max_screen_size, iteration):
        ...
        ##before_points = self.get_xyz.shape[0]
        ##print(f"[gaussian_densification] before. iteration: {iteration}, points: {before_points}")
        self.densify_and_clone(grads, max_grad, extent)
        ##print(f"[gaussian_densification_clone] after. iteration: {iteration}, points: {self.get_xyz.shape[0]}, change: {self.get_xyz.shape[0] - before_points}")
        ##before_points = self.get_xyz.shape[0]
        _prune_filter = self.densify_and_split(grads, max_grad, extent)
        ##print(f"[gaussian_densification_split] after. iteration: {iteration}, points: {self.get_xyz.shape[0]}, change: {self.get_xyz.shape[0] - before_points}")
        ...
        ##before_points = self.get_xyz.shape[0]
        ##print(f"[gaussian_pruning] before. iteration: {iteration}, points: {before_points}")
        self.prune_points(prune_mask)
        ##print(f"[gaussian_pruning] after. iteration: {iteration}, points: {self.get_xyz.shape[0]}, change: {self.get_xyz.shape[0] - before_points}")
