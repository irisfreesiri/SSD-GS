...
class Scene:
    ...
    def restore(self, camera_args):
        ...
        for name, data in camera_args["train_cameras"].items():
            cam = train_map.get(name)
            if cam is not None:
                (
                    cam.pl_adj,
                    cam.pl_pos,
        ...
        for name, data in camera_args["test_cameras"].items():
            cam = test_map.get(name)
            if cam is not None:
                (
                    cam.pl_adj,
                    cam.pl_pos,
