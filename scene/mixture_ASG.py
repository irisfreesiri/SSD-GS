...
class Mixture_of_ASG(nn.Module):
    ...
    def forward(self, wi, wo, alpha, asg_scales, asg_sigmas, asg_axises):
        """
        wi, wo: (K, 3)
        ...
        # variance
        lam = torch.clamp(asg_scales[:, 0], min=1e-4).unsqueeze(0)  # (1, basis_asg_num)
        miu = torch.clamp(asg_scales[:, 1], min=1e-4).unsqueeze(0)  # (1, basis_asg_num)
        sigma = torch.clamp(asg_sigmas, min=1e-4).unsqueeze(0)      # (1, basis_asg_num)
