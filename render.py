...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, gamma):
    ...
    # If the directories exist, remove them first
    for path in [render_path, gts_path]:
    ...
    for idx, view in enumerate(tqdm(views, desc="Rendering progress")):
        render_pkg = render(view, gaussians, light_stream, calc_stream, local_axises, asg_scales, asg_sigmas, asg_axises, pipeline, background)
        render_image = render_pkg["render"]
        gt_image = view.original_image[0:3, :, :]

        if gamma:
            gt_image = torch.pow(gt_image, 1/2.2)
            render_image = torch.pow(render_image, 1/2.2)
    ...
    with torch.no_grad():
        ...
        if dataset.use_nerual_phasefunc:
            ...
            if os.path.exists(_model_path):
                (model_params, camera_params, first_iter) = torch.load(_model_path)
                # load ASG parameters
                ...
                # Extend Scatter MLP
                gaussians.scatter_mlp = ScatterMLP(dataset.neural_material_size).to(device="cuda")
                gaussians.scatter_mlp.load_state_dict(model_params[12])
                gaussians.scatter_mlp.eval()
                # load MLP parameters
                ...
                gaussians.neural_phasefunc.load_state_dict(model_params[16])
